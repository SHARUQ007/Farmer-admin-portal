...
export default function NonScheduledTransporter({data,isLoading,errorMsg,meta,fetchNonScheduledTransporter}) {
  ...
  const handleChangePage = async (event, newPage) => {
        ...
        fetchNonScheduledTransporter(newPage + 1, rowsPerPage)
  }
  ...
  const handleChangeRowsPerPage = async (event) => {
        if (event.target.value){
            ...
            await setPage(0);
            fetchNonScheduledTransporter(1, val)
        }
  }
  if(data.length>0){
